...
void PayloadControl::Pickup()
{
    ...
    case State::UNSPOOL:
        ...
        { // check load with timeout to prevent drift
        ...
        if (stopDrop_)
        {
            if (millis() - waitTimerStart_ <= 350)
            { // wait for 500 ms for some slack
                break;
            }
...
void PayloadControl::Dispense()
{
    ...
    case State::WAIT:
        ...
        {
            dispenseAmount_ = 9999; // dispense everything
        }
        // logic here to check water levels
        // if time is exceeded or enough water is dispensed
        if (millis() - waitTimerStart_ >= dispenseTime_ * 1000 || (abs(lastWeight_ - weight_) > dispenseAmount_ - 150 && !loadCellIsTweaking_))
        {
            SwitchState(State::RESPOOL);
    ...
    case State::RESPOOL:
        ...
        if (force1_ >= forceThreshold_ + 5 && force2_ >= forceThreshold_ + 5 || (millis() - lastEncoderChangeTime_) > 500)
        { // if force is detected, stop
            StopRetraction();
            numBuckets_ = (numBuckets_ > 1) ? numBuckets_ - 1 : 1; // decrement number of buckets
        }
        break;
    }
    return;
...
void PayloadControl::Reset()
{
    ...
    case State::RESPOOL:
        ...
        if (force1_ >= forceThreshold_ && force2_ >= forceThreshold_ || (millis() - lastEncoderChangeTime_) > 500)
        { // if force is detected, stop
            StopRetraction();
        }
    default:
